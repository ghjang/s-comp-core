<script lang="ts">
	...
	import * as loader from '../common/util/client/loader/ComponentLoader.js';
	...
	async function loadComponents(targetItems: Array<ToggleItem>): Promise<void> {
		...
		for (let i = 0; i < targetItems.length; ++i) {
			componentItems.push(targetItems[i] as ComponentItem);
			if (componentItems[i].component === undefined) {
				componentItems[i].component = null;
			}
		}

		await loader.loadAll(componentItems);
	...
	$: !isAllComponentsLoaded && loadComponents(items);
