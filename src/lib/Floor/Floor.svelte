<script lang="ts">
	...
	export const getCurrentChildComponentInfo = () => floorChild?.getChildComponentInfo();
	...
	onMount(async () => {
		const ancestorFloorContainer = findClosestAncestor(floorContainer, 'floor-container');
	...
	async function handleMenuItemClicked(event) {
		if (event.detail.link) {
			...
			if (!handler || typeof handler !== 'function') {
				if (defaultActionHandler && typeof defaultActionHandler === 'function') {
					handler = () => defaultActionHandler(event.detail.action);
				} else {
					handler = () => {
						console.warn(`no proper action menu item handler: ${event.detail.action.text}`);
					};
				}
			}
	...
	function handleHighlightFloor(event) {
		...
		if (highlighted) {
			floorContainer.addEventListener('animationend', () => (highlighted = false), { once: true });
		}
	}
...
>
	...
	{:else}
		<!-- svelte-ignore a11y-no-static-element-interactions -->
		<div class="floor-box {pattern}" on:contextmenu|stopPropagation={handleContextMenu}>
			<FloorChild
				bind:this={floorChild}
				{componentScriptBasePath}
				{floorLevel}
